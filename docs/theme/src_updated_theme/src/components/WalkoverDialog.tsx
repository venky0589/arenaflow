...
export function WalkoverDialog({ open, onClose, onConfirm, match }: WalkoverDialogProps) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to mark walkover')
    } finally {
      setLoading(false)
    }
  }
    ...
    <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent>
        <Stack spacing={2} sx={{ mt: 1 }}>
          <Alert severity="warning">
            This action is irreversible. The match will be marked as completed with WALKOVER status.
          </Alert>
          ...
          <TextField
            label="Reason"
            placeholder="e.g., Player did not show up"
            multiline
            rows={3}
            fullWidth
            required
            ...
            disabled={loading}
            helperText="Explain why this match ended in a walkover"
          />
      ...
      <DialogActions>
        ...
        <Button
          ...
          disabled={loading}
        >
          {loading ? 'Processing...' : 'Mark Walkover'}
        </Button>
      </DialogActions>
    </Dialog>
  )
